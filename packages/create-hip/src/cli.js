export function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    switch (arg) {
      ...
      case "--serve":
        ...
        break;
      case "--port":
        options.port = requireValue(argv, ++index, "--port");
        break;
      case "--validate":
      ...
      default:
        throw new Error(
          `Unknown argument '${arg}'. Run --help to see supported options.`,
        );
    }
  }
  ...
  if (options.bootstrap && (options.defaults || options.preset)) {
    throw new Error(
      "--bootstrap cannot be combined with --defaults or --preset.",
    );
  }
...
Options:
  ...
  --serve                Serve the bootstrap intake form from localhost
  --port <number>        Port to use when serving the intake form
...
export function formatValidationReport(report, options = {}) {
  ...
  if (report.merged) {
    lines.push("");
    lines.push(
      `Merged effective HIP: ${report.merged.valid ? "valid" : "invalid"}`,
    );
